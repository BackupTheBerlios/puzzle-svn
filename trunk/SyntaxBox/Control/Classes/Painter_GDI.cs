...
namespace Puzzle.Windows.Forms.SyntaxBox.Painter
{
    ...
    public class Painter_GDI : IPainter
    {
            ...
            if (Control != null)
            {
                if (Control.IsHandleCreated)
                {
                    ...
                    Control.View.RowHeight = GFX.BackBuffer.MeasureTabbedString("ABC", 0)
                                                 .Height + Control._SyntaxBox.RowPadding;
        ...
        public void RenderAll(Graphics g)
        {
            try
            {
                ...
                if (Control.SmoothScroll)
                {
                    if (diff == 1)
                    {
                        for (int i = Control.View.RowHeight;
                             i > 0;
                             i -=
                             Control.SmoothScrollSpeed)
                        {
                              ...
                              Control.View.TextMargin)
                {
                    ...
                    return ColumnFromPixel(xtr.Expansion_EndRow.Index, X -
                                                                       ...
                                                                       MeasureRow(xtr.Expansion_EndRow,
                                                                                  xtr.Expansion_EndRow.
                                                                                      Expansion_StartChar,
                                                                                  xtr.Expansion_EndRow.
                                                                                      Expansion_PixelStart)
                                                                           .Width);
                }
                else
                {
                ...
                foreach (Word w in xtr.FormattedWords)
                {
                                                           ...
                                                           xtr.FormattedWords[xtr.FormattedWords.Count - 1])
                    {
                        ...
                        TotWidth += GFX.StringBuffer.DrawTabbedString(w.Text.Substring(0,
                                                                                       MaxChars), xpos + TotWidth, 0,
                                                                      taborig, Control.PixelTabSize)
                            .Width;
                        width = TotWidth;
                        break;
                    }
                    else
                    {
                        TotWidth += GFX.StringBuffer.DrawTabbedString(w.Text, xpos +
                                                                              TotWidth, 0, taborig, Control.PixelTabSize)
                            .Width;
            ...
            if (CurrWord != null)
            {
                ...
                if (CurrentSegment != null)
                {
                    ...
                    try
                    {
                        ...
                        {
                            Word w = Control.Document.GetStartBracketWord(CurrWord,
                        ...
                        {
                            Word w = Control.Document.GetEndBracketWord(CurrWord,
                                                                        CurrWord.Pattern.MatchingBracket,
                                                                        CurrWord.Segment);
        ...
        private void SetSpanIndicators()
        {
            ...
            try
            {
                Segment s = Control.Caret.CurrentSegment();
        ...
        private void RenderAll2(Graphics g)
        {
            ...
            {
                //			Console.WriteLine ("RenderAll failure:" + x.Message );
            }
        }
        ...
        private void RenderRow(Graphics g, int RowIndex, int RowPos)
        {
            ...
            try
            {
                ...
                if (Control.ContainsFocus || Control.View.Action ==
                ...
                var rec = new Rectangle(0, 0, Control.Width,
            ...
            try
            {
                if (r.Expansion_StartSegment.EndRow != null)
                {
                    if (r.Expansion_StartSegment.EndRow.RowState ==
                        ...
                        Control.Document.Parser.ParseLine
        ...
        private void RenderText(int RowIndex, int XOffset, Word StartWord)
        {
            ...
            if (RowIndex <= Control.Document.Count - 1)
            {
                ...
                foreach (Word w in xtr.FormattedWords)
                {
                    if (StartDraw)
                    {
                            ...
                            !DrawBreakpoint && Control.ShowTabGuides)
                        {
                            ...
                            if ((xtab/(double) Control.PixelTabSize) == (xtab/
                        ...
                        if (w.Type == WordType.xtWord || ws == false)
                        {
                            ...
                            if (Control.BracketMatching && (w == BracketEnd || w ==
                                                                               BracketStart))
                            {
                        ...
                        if (w.Pattern != null)
                            ...
                            {
                                bbuff.FillRect(Control.SeparatorColor,
                ...
                if (xtr.IsCollapsed) {}
                else if (xtr.EndSegment != null && xtr.EndSegment.BlockType != null &&
                         xtr.EndSegment.BlockType.Style != null)
                {
                    bbuff.FillRect(xtr.EndSegment.BlockType.Style.BackColor, xpos, 0, Control.Width - xpos,
                                   Control.View.RowHeight);
                }
        ...
        private void RenderSelectedText(int RowIndex, int XOffset, Word StartWord)
        {
            ...
            if (RowIndex <= Control.Document.Count - 1)
            {
                ...
                if (xtr.IsCollapsed) {}
                         ...
                         xtr.EndSegment.BlockType.Style != null)
                {
                    GFX.BackBuffer.FillRect(xtr.EndSegment.BlockType.Style.BackColor, xpos, 0, Control.Width - xpos,
                                            Control.View.RowHeight);
                }
        ...
        private void RenderCaret(int RowIndex, int ypos)
        {
            ...
            else
            {
                ...
                if (!Control.OverWrite)
                {
                    ...
                    if (Control.IMEWindow == null)
                    {
                        Control.IMEWindow = new IMEWindow
                            (Control.Handle, Control.FontName,
                             Control.FontSize);
                        InitIMEWindow();
                    }
        ...
        private void RenderMargin(int RowIndex)
        {
            ...
            if (Control.ShowLineNumbers)
            {
                ...
                if (RowIndex <= Control.Document.Count - 1)
                {
                    int nw = MeasureString((RowIndex + 1).ToString
                                               (CultureInfo.InvariantCulture)).Width;

                    bbuff.DrawTabbedString((RowIndex + 1).ToString
                                               (CultureInfo.InvariantCulture),
                                           Control.View.GutterMarginWidth + Control.View.LineNumberMarginWidth - nw - 1,
                                           1, 0, Control.PixelTabSize);
                }
            }
        }
                                 ...
                                 Color.Transparent && Control._SyntaxBox.ShowScopeIndicator)
                {
                    if (RowIndex >= FirstSpanRow && RowIndex <= LastSpanRow)
                        bbuff.FillRect(Control._SyntaxBox.ScopeIndicatorColor,
                                       Control.View.TotalMarginWidth + 14, 0, 2,
                                       Control.View.RowHeight);

                    if (RowIndex == FirstSpanRow)
                        bbuff.FillRect(Control._SyntaxBox.ScopeIndicatorColor,
                                       Control.View.TotalMarginWidth + 14, 0, 4, 2);

                    if (RowIndex == LastSpanRow)
                        bbuff.FillRect(Control._SyntaxBox.ScopeIndicatorColor,
                                       Control.View.TotalMarginWidth + 14,
                ...
                if (!xtr.IsCollapsed)
                {
                        ...
                        (RowIndex < Control.Selection.LogicalBounds.LastRow))
                    {
                        int width = MeasureRow(xtr, xtr.Text.Length).Width +
                                    MeasureString("¶").Width + 3;
                        RenderBox(Control.View.TextMargin, 0, Math.Max(width -
                                                                       Control.View.ClientAreaStart, 0),
                                  Control.View.RowHeight,
                                  Invert);
                    }
        ...
        private TextPoint ColumnFromPixel(int RowIndex, int X)
        {
            ...
            foreach (char c in Word.Text)
            {
                int tmpWidth = GFX.StringBuffer.DrawTabbedString(c + "", xpos +
                                                                         WordStart, 0, taborig, Control.PixelTabSize).
                    Width;
            ...
            if (!found && Control.View.CharWidth > 0 &&
        ...
        {
            return GetTextPointPixelPos(Control.Caret.Position);
        }
