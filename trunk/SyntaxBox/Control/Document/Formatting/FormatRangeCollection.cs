...
{
    public class FormatRangeCollection : CollectionBase
    {
        #region PUBLIC PROPERTY DOCUMENT

        private readonly SyntaxDocument _Document;
        ...
        {
            if (RowContainsFormats(Document[yPos]))
            {
        ...
        {
            string Text = Document.GetRange(Range);
            string tmp = Text.Replace(Environment.NewLine, "\n");
            int l = tmp.Length;
            string[] tmplines = tmp.Split('\n');
