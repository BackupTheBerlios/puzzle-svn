...
using Puzzle.Windows.Forms.FormatLabel;
...
namespace Puzzle.Windows.Forms.CoreLib
{
	...
	public class FormatLabelControl : BaseControl
	{
		...
		private Element _ActiveElement = null;
...
//					}
			...
			Element[] res = new Element[Elements.Count];
			Elements.CopyTo(res);
			return res;
		}
		...
		private void ApplyFormat(Element[] Elements)
		{
			...
			foreach (Element Element in Elements)
			{
				...
				Element Link = (Element) link.Peek();
				...
				foreach (Element Element in _Elements)
				{
					...
					foreach (Word word in Element.words)
					{
						if (WordWrap)
						{
							...
							if ((word.Width + x) > this.ClientWidth - Margin - scrollwdh)
							{
				...
				foreach (Row r in this._Rows)
				{
					...
					if (index > 0)
					{
						...
						if (previndex >= 0)
						{
							...
							{
								Word w = (Word) prev.Words[prev.Words.Count - 1];
								height = w.Height;
							}
						}
		...
		public int GetWidth()
		{
			...
			return max + Margin*2 + this.BorderWidth*2;
		}
