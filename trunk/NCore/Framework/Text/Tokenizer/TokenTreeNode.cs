...
namespace Puzzle.NCore.Framework.Text
{
	public class TokenTreeNode
	{
		...
		public void AddExpression(string text, bool caseSensitive, bool needSeparators, IPatternMatcher matcher, object tag)
		{
			...
			if (text.Length == 1)
			{
				...
				patternMatcherReference.NextSibling = this.FirstExpression;
		...
		public void AddToken(string text, bool caseSensitive, bool needSeparators, object tag)
		{
			...
			else
			{
				string leftovers = text.Substring(1);
				char childChar = leftovers[0];
				int childIndex = (int) childChar & 0xff; //make a lookupindex (dont mind if unicode chars end up as siblings as ascii)
