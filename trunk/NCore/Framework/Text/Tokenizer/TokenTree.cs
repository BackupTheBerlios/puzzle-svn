...
namespace Puzzle.NCore.Framework.Text
{
	public class TokenTree
	{
		...
		public void AddExpression(string prefix, bool caseSensitive, bool needSeparators, IPatternMatcher matcher, object tag)
		{
		...
		private void AddExpressionWithCaseInsensitivePrefix(string prefix, bool needSeparators, IPatternMatcher matcher, object tag)
		{
		...
		private void AddExpressionWithCaseSensitivePrefix(string prefix, bool needSeparators, IPatternMatcher matcher, object tag)
		{
			char startChar = prefix[0];
			int startIndex = (int) startChar;
			if (nodes[startIndex] == null)
		...
		private void AddExpressionWithoutPrefix(IPatternMatcher matcher, bool caseSensitive, bool needSeparators, object tag)
		{
			if (matcher.DefaultPrefixes != null)
			{
				foreach (string defaultPrefix in matcher.DefaultPrefixes)
				{
					this.AddExpression(defaultPrefix, caseSensitive, needSeparators, matcher, tag);
				}
			}
			else
			{
		...
		public void AddToken(string text, object tag)
		{
			...
			this.AddToken(text, true, false, tag);
		}
		...
		public MatchResult Match(string text, int startIndex)
		{
			...
			for (int currentIndex = startIndex; currentIndex < textLength; currentIndex ++)
			{
				...
				for (int matchIndex = currentIndex + 1; matchIndex <= textLength; matchIndex++)
				{
					...
					node = node.ChildNodes[node.ContainsCaseInsensitiveData ? (int) CharUtils.ToLower(textLookup[(int) text[matchIndex]]) & 0xff : (int) textLookup[(int) text[matchIndex]] & 0xff];


					//if node is not null then do: if caseinsensitive then do: insensitivelookup else do: casesesnsitivelookup
					while (node != null ? (node.ContainsCaseInsensitiveData ? (node.Char != CharUtils.ToLower(textLookup[(int) text[matchIndex]])) : (node.Char != textLookup[(int) text[matchIndex]])) : false)
